...
history = model.fit(r.training_features, r.training_labels, epochs = 500,
...
cat(paste0("MAPE on the Training Data:\t", format(mape(training_labels[[1]], training_predictions[[1]])*100, digits=3, nsmall=2)))

cat(paste0("\nMAPE on the Validation Data:\t", format(mape(validation_labels[[1]], validation_predictions[[1]])*100, digits=3, nsmall=2)))
```
...
data_test <- data.frame(prediction = validation_predictions[,1], actual = validation_labels[,1])
data_test2 <- data.frame(prediction = testing_predictions[,1])
...
ggplot(data_test2) +
  geom_line( aes(x=1:length(prediction), y=prediction, colour = "Predicted Values" )) +
  labs(title="Prediction for the Test Data") +
  xlab("Case Number") +
  ylab("Price in EUR") 
```
